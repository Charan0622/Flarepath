...
// Fallback coord lookup for seeded demo addresses.
...
export async function GET(
  ...
  const { data: dispatch, error: dErr } = await db
    ...
    .single();
  if (dErr || !dispatch) return apiError("Dispatch not found", 404);

  const [incidentRes, vehicleRes, triageRes] = await Promise.all([
    db.from("incidents").select("id, type, severity, status, address, created_at, description, hazards, reporter_name, reporter_phone, location")
  ...
  if (!incident || !vehicle) return apiError("Incident or vehicle missing", 404);
  ...
  return apiSuccess({
    dispatch: {
      ...
      distance_m: dispatch.distance_m,
